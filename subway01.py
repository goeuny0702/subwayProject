...
초량_부산 = line(can, 555, 816, 490, 816, "orange")
부산_중앙 = line(can, 490, 816, 434, 816, "orange")
...
부산 = circle(can, 490, 816, 10, "부산", "orange", 'd')
...
subway_map = {
    ...
    '초량': {'region': (555, 816), 'connections': {'부산': ('1호선', 1, 10), '부산진': ('1호선', 1, 10)}},
    '부산': {'region': (490, 816), 'connections': {'중앙': ('1호선', 1, 25), '초량': ('1호선', 1, 10)}},
    '중앙': {'region': (434, 816), 'connections': {'남포': ('1호선', 1, 25), '부산': ('1호선', 1, 25)}},
...
# 1. 스크롤 가능 프레임을 만들기 위한 구조 설정
scroll_container = ttkb.Frame(canvas)
...
# 3. Scrollbar 생성 및 Canvas에 연결
v_scrollbar = ttk.Scrollbar(scroll_container, orient="vertical", command=scroll_canvas.yview)
v_scrollbar.pack(side="right", fill="y")
scroll_canvas.configure(yscrollcommand=v_scrollbar.set)

# 4. 실제 내용을 담을 프레임(result_container)
result_container = ttkb.Frame(scroll_canvas, bootstyle="light")
result_container_id = scroll_canvas.create_window((0, 0), window=result_container, anchor="nw")

# 5. 프레임 크기 변화에 따라 스크롤 영역 자동 갱신
def on_frame_configure(event):
    scroll_canvas.configure(scrollregion=scroll_canvas.bbox("all"))

result_container.bind("<Configure>", on_frame_configure)

# 6. 윈도우 크기 변경 시 프레임 너비를 Canvas에 맞춤
def resize_frame(event):
    canvas_width = event.width
    scroll_canvas.itemconfig(result_container_id, width=canvas_width)

scroll_canvas.bind("<Configure>", resize_frame)
...
def auto_scroll_smoothly(step=1, delay=30, limit=100):
    """
    step: 얼마나 스크롤할지 (작을수록 더 부드러움)
    delay: 각 스크롤 사이 딜레이 (ms)
    limit: 최대 몇 번 스크롤할지
    """
    count = 0
    ...
    def scroll():
        ...
        if count < limit:
            ...
            count += 1
            scroll_canvas.after(delay, scroll)
...
def search_route1():
    ...
    auto_scroll_smoothly(step=1, delay=1000, limit=60)  # 약 1~2초간 부드럽게 스크롤됨
